...
for i in range(len(images)):
    ...
    print("Image: {}, Label: {}, Confidence: {}".format(i+1, get_key(label, label_dict), confidence))
...
while True:
    ...
    if len(faces) > 0:
        if(len(faces)>1):
            ...
            draw_label(frame, (50, 50), "More than one face detected",colour=(0,0,255))
            ...
            if(confirmstatenum>=40):
                ...
                with open('votes.csv', 'r', newline='') as csvfile:
                    ...
                    if label_text in data:
                        print("Already voted")
                        draw_label(frame, (x, y), label_text+" Already voted",colour=(0,0,255))
                        confirmstatenum=0
                    else:
